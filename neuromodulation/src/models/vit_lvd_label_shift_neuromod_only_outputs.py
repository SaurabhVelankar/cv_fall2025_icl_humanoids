...
class SharedNAWeights(nn.Module):
    ...
    def forward(self, mean_dev: torch.Tensor, var_dev: torch.Tensor, site_id) -> torch.Tensor:
        B = mean_dev.size(0)

        if isinstance(site_id, int):
            emb = self.site_emb.weight[site_id].unsqueeze(0).expand(B, -1)
        elif torch.is_tensor(site_id):
            if site_id.dim() == 0 or site_id.numel() == 1:
                emb = self.site_emb.weight[int(site_id.item())].unsqueeze(0).expand(B, -1)
            else:
                emb = self.site_emb(site_id.to(dtype=torch.long, device=mean_dev.device))
        else:
            raise TypeError(f"Unsupported site_id type: {type(site_id)}")

        x = torch.cat([mean_dev, var_dev, emb.to(device=mean_dev.device)], dim=1)
        sal = self.net(x)
        return sal
...
class SharedAChWeights(nn.Module):
    ...
    def forward(self, x_gap: torch.Tensor, site_id) -> torch.Tensor:
        B = x_gap.size(0)

        # site_id can be int / 0-d tensor / (B,) tensor
        if isinstance(site_id, int):
            emb = self.site_emb.weight[site_id].unsqueeze(0).expand(B, -1)
        elif torch.is_tensor(site_id):
            if site_id.dim() == 0 or site_id.numel() == 1:
                ...
                emb = self.site_emb(site_id.to(dtype=torch.long, device=x_gap.device))
        else:
            raise TypeError(f"Unsupported site_id type: {type(site_id)}")

        z = torch.cat([x_gap, emb.to(device=x_gap.device)], dim=1)
        rel = self.net(z)
...
class MLPBatchNormSite(nn.Module):
    """BatchNorm on patch tokens only (used as an FFN/MLP forward-hook)."""
    def __init__(self, dim: int, num_register_tokens: int):
        super().__init__()
        self.bn = PatchTokenBatchNorm1d(dim=dim, num_register_tokens=num_register_tokens)

    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        # tokens: (B, 1+R+P, D)
        return self.bn(tokens)


class TokenNeuromodSite(nn.Module):
    """
    NA/ACh modulation applied to *all* tokens (CLS + registers + patches) at the end of a transformer block.
    Whitening is handled at patch projection; BN (patch-only) is handled separately via an FFN hook.
    """
    def __init__(
        self,
        dim: int,
    ...
    def _apply_na(self, x: torch.Tensor) -> torch.Tensor:
        if self.na_state is None:
            return x
        gain = self.na_state(x)              # (B, D)  (stats pooled across tokens)
        return x * gain.unsqueeze(1)         # (B, N, D)

    def _apply_ach(self, x: torch.Tensor) -> torch.Tensor:
        if self.ach_shared is None:
            return x
        x_gap = x.mean(dim=1)                # (B, D)
        gain = self.ach_shared(x_gap, self.site_index)  # (B, D)
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        if self.na_ach_parallel:
            ...
            if self.na_state is not None:
                outs.append(self._apply_na(tokens))
            if self.ach_shared is not None:
                outs.append(self._apply_ach(tokens))
            if outs:
...
class DINOv3ViTWithNeuromod(nn.Module):
    def __init__(
        ...
        if self.use_whitening:
            ...
            self.whitening = None
                ...
                # Token neuromod (end-of-block) + BN-in-FFN (hook on MLP output)
        ...
        if self.use_patch_na:
            self.patch_na_shared = SharedNAWeights(self.hidden_size, num_sites=self.num_layers)
        ...
        if self.use_patch_ach:
            self.patch_ach_shared = SharedAChWeights(self.hidden_size, num_sites=self.num_layers)
        else:
            self.patch_ach_shared = None

        # One neuromod site per transformer block (applied at block output)
        self.block_sites = nn.ModuleList([
            TokenNeuromodSite(
        ...
        # One BN site per block, applied to FFN/MLP module output (patch tokens only)
        # Enabled only when NA/ACh is enabled (to keep baseline + W unchanged).
        if self.use_patch_neuromod:
            self.mlp_bn_sites = nn.ModuleList([
                MLPBatchNormSite(dim=self.hidden_size, num_register_tokens=self.num_register_tokens)
    ...
    def _install_hooks(self):
        ...
        for b_idx, block in enumerate(blocks):
            # (1) BN hook on FFN/MLP output (patch tokens only)
            if self.use_patch_neuromod:
                mlp = _get_submodule(block, ("mlp", "ffn", "feed_forward", "intermediate"), ("mlp", "ffn"))
                bn_site = self.mlp_bn_sites[b_idx]

                def _make_mlp_bn_hook(bn_module: MLPBatchNormSite):
                    def _hook(_m, _inp, out):
                        if isinstance(out, tuple):
                            y0 = bn_module(out[0])
                            return (y0,) + out[1:]
                        return bn_module(out)
                    return _hook

                self._hooks.append(mlp.register_forward_hook(_make_mlp_bn_hook(bn_site)))

            # (2) Neuromod hook at end of block (all tokens)
            if self.use_patch_neuromod:
                site = self.block_sites[b_idx]

                def _make_block_hook(site_module: TokenNeuromodSite):
                    def _hook(_m, _inp, out):
                        if isinstance(out, tuple):
                ...
                self._hooks.append(block.register_forward_hook(_make_block_hook(site)))

        if self.use_patch_neuromod:
            print("[hooks] installed: FFN BN (patch-only) + end-of-block NA/ACh (all tokens)")
        else:
            print("[hooks] installed: whitening only (no NA/ACh)")
    ...
    @torch.no_grad()
    def forward_features_nograd(self, pixel_values: torch.Tensor) -> torch.Tensor:
        out = self.backbone(pixel_values=pixel_values, return_dict=True)
        x = out.last_hidden_state  # (B, 1+R+P, D)
    ...
    def get_trainable_params(self) -> List[nn.Parameter]:
        ...
        if self.use_patch_neuromod:
            params += list(self.mlp_bn_sites.parameters())
            if self.use_patch_na and self.patch_na_shared is not None:
                params += list(self.patch_na_shared.parameters())
            if self.use_patch_ach and self.patch_ach_shared is not None:
...
def extract_features_and_logits(model: DINOv3ViTWithNeuromod, loader: DataLoader, device: str, desc: str):
    ...
    for batch in tqdm(loader, desc=desc, leave=False):
        ...
        feats = model.forward_features_nograd(images)
