...
class COCO2017SingleLabelDataset(Dataset):
    ...
    Label strategy: dominant category by total annotated area (sums 'area' across instances).
    Optionally restrict to images containing exactly one category.
    """
    def __init__(
        self,
        coco_root: str,
        ...
        self.seed = seed
        self.single_category_only = single_category_only
        self.min_instances = min_instances

        self.images_dir = os.path.join(coco_root, split)
        self.ann_file = os.path.join(coco_root, "annotations", f"instances_{split}.json")

        if not os.path.isdir(self.images_dir):
            raise FileNotFoundError(f"Missing COCO images dir: {self.images_dir}")
        ...
        items = []  # list of (img_path, category_id)
        for img_id in img_ids:
            ...
            if self.single_category_only and len(cats) != 1:
                continue
        ...
        # deterministically subsample for speed
        rng = np.random.RandomState(seed)
        rng.shuffle(items)
        if self.max_images is not None and len(items) > self.max_images:
        ...
        self.idx_to_catid = {i: cid for cid, i in self.catid_to_idx.items()}
...
def main():
    ...
    parser.add_argument("--model_dir", type=str,
                        default="/scratch/vjh9526/cn_fall_2025/models")
    ...
    parser.add_argument("--ood_source", type=str, default="coco",
                        choices=["holdout", "coco", "both"],
                        help="Which OOD set to evaluate: ImageNet holdout classes, COCO, or both.")
    parser.add_argument("--coco_root", type=str, default="/scratch/vjh9526/cn_fall_2025/data/coco2017",
                        help="COCO root directory containing val2017/ and annotations/.")
    parser.add_argument("--coco_max_images", type=int, default=5000,
                        help="Max COCO images to use (for speed).")
    parser.add_argument("--coco_single_category_only", action="store_true",
                        help="If set, only keep COCO images containing exactly one category (cleaner labels, fewer images).")
    parser.add_argument("--coco_min_instances", type=int, default=1,
                        help="Minimum number of object instances required in a COCO image.")
    ...
    def eval_ood_set(prefix: str, ood_loader: DataLoader, id_logits: torch.Tensor):
        print(f"Extracting {prefix} features/logits...")
        ood_feats, ood_logits, ood_labels = extract_features_and_logits(
            model, ood_loader, desc=f"[{args.config}] {prefix} features"
        )
        ...
        # Geometry/cov/few-shot are computed on OOD features + (OOD) labels
        d_intra, d_inter, sep = compute_geometry_metrics(ood_feats, ood_labels)
        _, frob = compute_covariance_metrics(ood_feats)

        stage0_feats = extract_stage0_features(
            model, ood_loader, max_total_samples=200_000, desc=f"[{args.config}] stage0 {prefix}"
        )
        _, frob_stage0 = compute_covariance_metrics(stage0_feats)

        fewshot = few_shot_eval(ood_feats, ood_labels, desc_prefix=f"{args.config} {prefix} few-shot")
        ...
        for k_shot, acc in fewshot.items():
            out[f"{prefix}_fewshot_{k_shot}"] = acc
    ...
    # ID accuracy (on known-class eval)
    ...
    # Holdout (ImageNet) OOD evaluation (existing)
    if args.ood_source in ("holdout", "both"):
        row.update(eval_ood_set("HOLDOUT", ood_loader, id_logits))

    # COCO OOD evaluation
    if args.ood_source in ("coco", "both"):
        if coco_loader is None:
            raise RuntimeError("coco_loader was not created but ood_source requires COCO.")
        row.update(eval_ood_set("COCO", coco_loader, id_logits))
