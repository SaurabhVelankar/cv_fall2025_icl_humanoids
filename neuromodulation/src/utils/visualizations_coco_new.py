...
def compute_summary_scores(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
    out = df.copy()

    msp_auroc = f"{prefix}_MSP_AUROC"
    eng_auroc = f"{prefix}_ENG_AUROC"
    msp_fpr95 = f"{prefix}_MSP_FPR95"
    eng_fpr95 = f"{prefix}_ENG_FPR95"

    # Means for convenience
    if {msp_auroc, eng_auroc}.issubset(out.columns):
        out[f"{prefix}_AUROC_mean"] = (_as_float_series(out, msp_auroc) + _as_float_series(out, eng_auroc)) / 2.0

    if {msp_fpr95, eng_fpr95}.issubset(out.columns):
        out[f"{prefix}_FPR95_mean"] = (_as_float_series(out, msp_fpr95) + _as_float_series(out, eng_fpr95)) / 2.0

    # Composite (higher better)
    if {f"{prefix}_AUROC_mean", f"{prefix}_FPR95_mean"}.issubset(out.columns):
        out[f"{prefix}_OOD_composite"] = (
            out[f"{prefix}_AUROC_mean"].astype(float) + (1.0 - out[f"{prefix}_FPR95_mean"].astype(float))
        ) / 2.0
...
def main():
    ...
    # Save raw table outputs for the report
    csv_path = os.path.join(args.out_dir, "all_results.csv")
    df.to_csv(csv_path, index=False)
    print(f"Loaded {len(df)} configs. Wrote: {csv_path}")

    # Plot ID_acc once (not prefix-specific)
    if "ID_acc" in df.columns:
        df_id = order_df(df.copy(), "ID_acc", ascending=False)
        dotplot_sorted(
            df_id,
            metric="ID_acc",
            out_path=os.path.join(args.out_dir, "00_id_accuracy_dot.png"),
            title="ID Accuracy (Known Classes) — dot/lollipop",
            xlabel="Accuracy",
            higher_is_better=True,
        )

    # Detect which OOD prefixes exist in the JSON keys
    prefixes = []
    for p in ["HOLDOUT", "COCO"]:
        if any(col.startswith(p + "_") for col in df.columns):
            prefixes.append(p)

    # Plot a full set of figures per OOD source
    for p in prefixes:
        df_p = compute_summary_scores(df, prefix=p)  # <-- note: compute_summary_scores must accept prefix now

        # Map generic order_by to prefix-specific column
        order_metric = args.order_by
        if order_metric == "OOD_composite":
            order_metric = f"{p}_OOD_composite"

        df_plot = order_df(df_p, order_metric, ascending=args.ascending)

        # AUROC paired dots
        if {f"{p}_MSP_AUROC", f"{p}_ENG_AUROC"}.issubset(df_plot.columns):
            paired_dotplot_sorted(
                df_plot,
                metric_a=f"{p}_MSP_AUROC",
                metric_b=f"{p}_ENG_AUROC",
                label_a=f"{p}_MSP_AUROC",
                label_b=f"{p}_ENG_AUROC",
                out_path=os.path.join(args.out_dir, f"{p}_01_ood_auroc_paired_dot.png"),
                title=f"{p}: OOD Detection AUROC — paired dots (higher is better)",
                xlabel="AUROC",
                higher_is_better=True,
                sort_by="mean",
            )
        ...
        # FPR95 paired dots
        if {f"{p}_MSP_FPR95", f"{p}_ENG_FPR95"}.issubset(df_plot.columns):
            paired_dotplot_sorted(
                df_plot,
                metric_a=f"{p}_MSP_FPR95",
                metric_b=f"{p}_ENG_FPR95",
                label_a=f"{p}_MSP_FPR95",
                label_b=f"{p}_ENG_FPR95",
                out_path=os.path.join(args.out_dir, f"{p}_02_ood_fpr95_paired_dot.png"),
                title=f"{p}: OOD Detection FPR@95TPR — paired dots (lower is better)",
                xlabel="FPR@95TPR",
        ...
        # Separation
        if f"{p}_sep" in df_plot.columns:
            dotplot_sorted(
                df_plot,
                metric=f"{p}_sep",
                out_path=os.path.join(args.out_dir, f"{p}_03_feature_separation_dot.png"),
                title=f"{p}: Feature Separation (d_inter / d_intra) — dot/lollipop (higher is better)",
                xlabel="Separation",
                higher_is_better=True,
            )

        # Covariance Frobenius
        if f"{p}_frob_cov" in df_plot.columns:
            dotplot_sorted(
                df_plot,
                metric=f"{p}_frob_cov",
                out_path=os.path.join(args.out_dir, f"{p}_04_cov_frobenius_dot.png"),
                title=f"{p}: Covariance Distance to Identity (Frobenius) — dot/lollipop (lower is better)",
                xlabel="||Cov - I||_F",
                higher_is_better=False,
            )

        if f"{p}_frob_cov_stage0" in df_plot.columns:
            dotplot_sorted(
                df_plot,
                metric=f"{p}_frob_cov_stage0",
                out_path=os.path.join(args.out_dir, f"{p}_05_cov_frobenius_stage0_dot.png"),
                title=f"{p}: Stage0 Covariance Distance to Identity (Frobenius) — dot/lollipop (lower is better)",
                xlabel="||Cov - I||_F",
                higher_is_better=False,
            )

        # Few-shot curves: rename prefix columns into the old names temporarily
        few_cols = [c for c in df_plot.columns if c.startswith(f"{p}_fewshot_")]
        if few_cols:
            df_tmp = df_plot.copy()
            for c in few_cols:
                df_tmp[c.replace(f"{p}_", "")] = df_tmp[c]  # COCO_fewshot_5 -> fewshot_5
            fewshot_lineplot(
                df_tmp,
                os.path.join(args.out_dir, f"{p}_06_fewshot_curves.png"),
                title=f"{p}: OOD Few-shot Linear Probe Accuracy Curves",
            )
        ...
        heatmap_metrics = [
            ...
            (f"{p}_MSP_AUROC", True),
            (f"{p}_ENG_AUROC", True),
            (f"{p}_MSP_FPR95", False),
            (f"{p}_ENG_FPR95", False),
            (f"{p}_sep", True),
            (f"{p}_frob_cov", False),
            (f"{p}_frob_cov_stage0", False),
        ]
        for k in [1, 5, 10, 15]:
            c = f"{p}_fewshot_{k}"
        ...
        heatmap_configs_by_metrics(
            ...
            out_path=os.path.join(args.out_dir, f"{p}_07_heatmap.png"),
            title=f"{p}: Configs × Metrics Heatmap (z-scored; higher=better orientation)",
        )
